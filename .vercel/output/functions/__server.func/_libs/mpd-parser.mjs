...
import { S as require_window, i as forEachMediaGroup, x as resolveUrl, y as decodeB64ToUint8Array } from "./global+videojs__vhs-utils.mjs";
import { t as require_lib } from "./xmldom__xmldom.mjs";
//#region node_modules/mpd-parser/dist/mpd-parser.es.js
/*! @name mpd-parser @version 1.4.0 @license Apache-2.0 */
var import_window = /* @__PURE__ */ __toESM(require_window());
...
var merge = (...objects) => {
	return objects.reduce((result, source) => {
		...
		Object.keys(source).forEach((key) => {
			...
			else if (isObject(result[key]) && isObject(source[key])) result[key] = merge(result[key], source[key]);
...
*   to m3u8-parser
*/
var urlTypeToSegment = ({ baseUrl = "", source = "", range = "", indexRange = "" }) => {
	const segment = {
		uri: source,
		resolvedUri: resolveUrl(baseUrl || "", source)
	};
	if (range || indexRange) {
		const ranges = (range ? range : indexRange).split("-");
		let startRange = import_window.default.BigInt ? import_window.default.BigInt(ranges[0]) : parseInt(ranges[0], 10);
		let endRange = import_window.default.BigInt ? import_window.default.BigInt(ranges[1]) : parseInt(ranges[1], 10);
		...
		let length;
		if (typeof endRange === "bigint" || typeof startRange === "bigint") length = import_window.default.BigInt(endRange) - import_window.default.BigInt(startRange) + import_window.default.BigInt(1);
...
var byteRangeToString = (byterange) => {
	let endRange;
	if (typeof byterange.offset === "bigint" || typeof byterange.length === "bigint") endRange = import_window.default.BigInt(byterange.offset) + import_window.default.BigInt(byterange.length) - import_window.default.BigInt(1);
...
var segmentRange = {
	...
	*/
	dynamic(attributes) {
		...
		const periodDuration = now + minimumUpdatePeriod - periodStartWC;
...
var addSidxSegmentsToPlaylist$1 = (playlist, sidx, baseUrl) => {
	...
	let startIndex;
	if (typeof sidx.firstOffset === "bigint") startIndex = import_window.default.BigInt(sidxEnd) + sidx.firstOffset;
	...
	for (let i = 0; i < mediaReferences.length; i++) {
		...
		let endIndex;
		if (typeof startIndex === "bigint") endIndex = startIndex + import_window.default.BigInt(size) - import_window.default.BigInt(1);
		...
		if (typeof startIndex === "bigint") startIndex += import_window.default.BigInt(size);
...
*/
var updateMediaSequenceForPlaylist = ({ playlist, mediaSequence }) => {
...
*/
var updateSequenceNumbers = ({ oldPlaylists, newPlaylists, timelineStarts }) => {
	newPlaylists.forEach((playlist) => {
		...
		if (oldPlaylist.segments[oldMatchingSegmentIndex].discontinuity && !firstNewSegment.discontinuity) {
...
*/
var positionManifestOnTimeline = ({ oldManifest, newManifest }) => {
...
var formatAudioPlaylist = ({ attributes, segments, sidx, mediaSequence, discontinuitySequence, discontinuityStarts }, isAudioOnly) => {
	const playlist = {
		attributes: {
			NAME: attributes.id,
...
var formatVttPlaylist = ({ attributes, segments, mediaSequence, discontinuityStarts, discontinuitySequence }) => {
...
var formatVideoPlaylist = ({ attributes, segments, sidx, discontinuityStarts }) => {
	const playlist = {
		attributes: {
			NAME: attributes.id,
			AUDIO: "audio",
	...
	if (attributes.qualityRanking !== void 0) playlist.attributes.SCORE = 1 / (attributes.qualityRanking + 1);
	if (attributes.contentProtection) playlist.contentProtection = attributes.contentProtection;
	if (attributes.serviceLocation) playlist.attributes.serviceLocation = attributes.serviceLocation;
	if (sidx) playlist.sidx = sidx;
	return playlist;
};
var videoOnly = ({ attributes }) => attributes.mimeType === "video/mp4" || attributes.mimeType === "video/webm" || attributes.contentType === "video";
var audioOnly = ({ attributes }) => attributes.mimeType === "audio/mp4" || attributes.mimeType === "audio/webm" || attributes.contentType === "audio";
var vttOnly = ({ attributes }) => attributes.mimeType === "text/vtt" || attributes.contentType === "text";
/**
...
var toM3u8 = ({ dashPlaylists, locations, contentSteering, sidxMapping = {}, previousManifest, eventStream }) => {
	...
	manifest.timelineStarts = getUniqueTimelineStarts(formattedPlaylists.map(({ timelineStarts }) => timelineStarts));
	...
	for (let sIndex = 0; sIndex < segmentTimeline.length; sIndex++) {
		...
		if (repeat < 0) {
			...
			if (nextS === segmentTimeline.length) {
				if (type === "dynamic" && minimumUpdatePeriod > 0 && media.indexOf("$Number$") > 0) count = getLiveRValue(attributes, time, duration);
				else count = (sourceDuration * timescale - time) / duration;
...
*/
var identifierReplacement = (values) => (match, identifier, format, width) => {
	if (match === "$$") return "$";
...
*/
var constructTemplateUrl = (url, values) => url.replace(identifierPattern, identifierReplacement(values));
/**
...
var segmentsFromTemplate = (attributes, segmentTimeline) => {
	...
	return parseTemplateInfo(attributes, segmentTimeline).map((segment) => {
...
var segmentsFromList = (attributes, segmentTimeline) => {
	...
	return segmentTimeInfo.map((segmentTime, index) => {
...
var generateSegments = ({ attributes, segmentInfo }) => {
	...
	} else if (segments.length) segmentAttributes.duration = segments.reduce((max, segment) => {
...
var findChildren = (element, name) => from(element.childNodes).filter(({ tagName }) => tagName === name);
...
var parseDuration = (str) => {
	...
	const match = /P(?:(\d*)Y)?(?:(\d*)M)?(?:(\d*)D)?(?:T(?:(\d*)H)?(?:(\d*)M)?(?:([\d.]*)S)?)?/.exec(str);
	if (!match) return 0;
	const [year, month, day, hour, minute, second] = match.slice(1);
	return parseFloat(year || 0) * SECONDS_IN_YEAR + parseFloat(month || 0) * SECONDS_IN_MONTH + parseFloat(day || 0) * SECONDS_IN_DAY + parseFloat(hour || 0) * SECONDS_IN_HOUR + parseFloat(minute || 0) * SECONDS_IN_MIN + parseFloat(second || 0);
};
var parseDate = (str) => {
	if (/^\d+-\d+-\d+T\d+:\d+:\d+(\.\d+)?$/.test(str)) str += "Z";
...
* the original casing.
*/
var attributeNameMap = { qualityranking: "qualityRanking" };
/**
...
var buildBaseUrls = (references, baseUrlElements) => {
	...
	return flatten(references.map(function(reference) {
		return baseUrlElements.map(function(baseUrlElement) {
			...
			const finalBaseUrl = merge(parseAttributes(baseUrlElement), { baseUrl: resolvedBaseUrl });
...
var getSegmentInformation = (adaptationSet) => {
	...
	const segmentUrls = segmentList && findChildren(segmentList, "SegmentURL").map((s) => merge({ tag: "SegmentURL" }, parseAttributes(s)));
	...
	if (template && segmentInitialization) template.initialization = segmentInitialization && parseAttributes(segmentInitialization);
	else if (template && template.initialization) template.initialization = { sourceURL: template.initialization };
	const segmentInfo = {
		template,
		...
		base: segmentBase && merge(parseAttributes(segmentBase), { initialization: parseAttributes(segmentInitialization) })
	};
...
var inheritBaseUrls = (adaptationSetAttributes, adaptationSetBaseUrls, adaptationSetSegmentInfo) => (representation) => {
	const repBaseUrls = buildBaseUrls(adaptationSetBaseUrls, findChildren(representation, "BaseURL"));
...
var parseCaptionServiceMetadata = (service) => {
	if (service.schemeIdUri === "urn:scte:dash:cc:cea-608:2015") return (typeof service.value !== "string" ? [] : service.value.split(";")).map((value) => {
		let channel;
		let language;
		language = value;
	...
	else if (service.schemeIdUri === "urn:scte:dash:cc:cea-708:2015") return (typeof service.value !== "string" ? [] : service.value.split(";")).map((value) => {
		const flags = {
			"channel": void 0,
			"language": void 0,
			"aspectRatio": 1,
		...
		if (/=/.test(value)) {
			...
			opts.split(",").forEach((opt) => {
				...
				if (name === "lang") flags.language = val;
				else if (name === "er") flags.easyReader = Number(val);
...
var toRepresentations = (periodAttributes, periodBaseUrls, periodSegmentInfo) => (adaptationSet) => {
	...
	let attrs = merge(periodAttributes, adaptationSetAttributes, { role: parseAttributes(role) });
	const accessibility = findChildren(adaptationSet, "Accessibility")[0];
	const captionServices = parseCaptionServiceMetadata(parseAttributes(accessibility));
	if (captionServices) attrs = merge(attrs, { captionServices });
	...
	if (Object.keys(contentProtection).length) attrs = merge(attrs, { contentProtection });
...
var toAdaptationSets = (mpdAttributes, mpdBaseUrls) => (period, index) => {
	...
	const periodAttributes = merge(mpdAttributes, { periodStart: period.attributes.start });
...
var generateContentSteeringInformation = (contentSteeringNodes, eventHandler) => {
	...
	const infoFromContentSteeringTag = merge({ serverURL: getContent(contentSteeringNodes[0]) }, parseAttributes(contentSteeringNodes[0]));
...
*/
var getPeriodStart = ({ attributes, priorPeriodAttributes, mpdType }) => {
...
var stringToMpdXml = (manifestString) => {
	...
	const parser = new import_lib.DOMParser();
	let xml;
	let mpd;
	try {
...
var parseUTCTimingScheme = (mpd) => {
	...
	switch (attributes.schemeIdUri) {
		...
		case "urn:mpeg:dash:utc:direct:2012":
			...
			break;
		default: throw new Error(errors.UNSUPPORTED_UTC_TIMING_SCHEME);
	}
	return attributes;
};
var parse = (manifestString, options = {}) => {
	...
	return toM3u8({
		dashPlaylists: toPlaylists(parsedManifestInfo.representationInfo),
...
//#endregion
export { parseUTCTiming as i, generateSidxKey as n, parse as r, addSidxSegmentsToPlaylist$1 as t };
